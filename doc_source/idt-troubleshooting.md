...
|  117  |  WriteFileFailed  |  Failed to write file \(can be the DUT or test machine\)\.  |  Try to manually create a file in the directory specified in the error message\. If you are running AWS IoT Greengrass in a Docker container, make sure your test machine has a `tmp` folder under the root directory\.  | 
